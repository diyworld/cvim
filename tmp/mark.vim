...
"command! 定义一个命令 Mark, 这个命令允许拥有直接给定的字符串参数
"-nargs=? 定义这个命令可以有0个或1个参数
"<f-args> 表示命令参数
"这个命令映射到函数 s:DoMark()
command! -nargs=? Mark call s:DoMark(<f-args>)

"autocmd! 定义一个事件触发的操作
"BufWinEnter buffer文件载入到窗口事件
"* 表示任何文件载入到窗口时执行
"call s:UpdateMark() 执行操作, 更新标记
...
function! s:InitMarkVariables()
  ...
  "g:mwCycleMax 标记组最大数
	if !exists("g:mwCycleMax")
		let i = 1
    "hlexists() 如果 MarkWord 存在则返回1
		while hlexists("MarkWord" . i)
			let i = i + 1
		endwhile
		let g:mwCycleMax = i - 1
	endif
  "g:wmCycle 当前标记组索引，指向下一个未使用的组
	if !exists("g:mwCycle")
		let g:mwCycle = 1
	endif
	let i = 1
...
" 返回当前光标下或光标之前的字符串, !表示允许覆盖同名函数
function! s:PrevWord()
  "获取当前行内容
	let line = getline(".")
  "获取当前光标所在的字符
  "如果该字符符合模式'\w', 既[0-9A-Za-z]
	if line[col(".") - 1] =~ '\w'
    "返回光标所在的单词
		return expand("<cword>")
	else
    "返回当前光标向左最近的一个单词
    "strpart(...) 获取当前光标及其之前的所有字符
    "'^.\{-}\(\w\+\)\W*$' 分解为 ^.{-}, \(\w\+\), \W*$ 共3个部分
    "                     第2部分用括号括起来, 表示要获取的信息, 用 '\1' 表示
    "                     第1和第3部分分别表示匹配但不需要的信息
    " ^  第一个非空字符
    " .  代表任意字符
    "\{-} 类似于 *, 匹配任意字符, 不同的是使用最短匹配算法
    "\w+ 匹配至少一个字符的数字字母组成的字符串
    "\W* 匹配0个或多个非数字字母组成的字符串
    " $  代表结束符<End>
...
function! s:DoMark(...) " DoMark(regexp)
  ...
  "a:0 表示参数个数
	...
	if regexp == ""
        ...
        "取消所有高亮的群组
        "MarkWork 高亮的群组名
        "syntax 是 syntax-highlighting 的缩写
  ...
  " 用一个特殊的包装字符，把 regexp 包起来
  " 这个循环可以保证选择一个 regexp 自身不包含的字符作为包装符
  " ex1: hello -> /hello/
  " ex2: h/hello -> ?h/hello?
  " ex3: ab/?~! -> @ab/?~!@
	let quote = "/?~!@#$%^&*+-=,.:"
  ...
  " 选择一个没有被标记的标记组
	...
	while i <= g:mwCycleMax
      ...
      " ?? 这个指令好像不能取消高亮呢?
			...
			" exe "windo syntax match MarkWord" . i . " " . quoted_regexp . " =ALL"
      " windo 对所有窗口执行后面的指令
      " highlight my_group ctermbg=green guibg=green 定义高亮组规则 my_group
      " match my_group /hello/  高亮 hello 字符串，高亮方式由 my_group 定义
      " match my_group // 取消高亮
